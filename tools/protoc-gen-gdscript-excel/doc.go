...
1. `tools/protoc-gen-gdscript/godot`, because the wrappers call protobuf helper
   classes such as `ProtoUtils` and `ProtoInputFile`.
2. `tools/protoc-gen-gdscript-excel/godot`, because chunk loading, index
...
1. `tools/protoc-gen-gdscript/godot`，因为包装器会调用 `ProtoUtils`、
   `ProtoInputFile` 等 protobuf 基础能力。
2. `tools/protoc-gen-gdscript-excel/godot`，因为分块加载、索引值转换和
   二分查找等辅助逻辑都位于这个运行时目录中。

这些运行时脚本不要求放在固定目录名下。实际项目里通常会把两层运行时都统一
放到 `addons/<name>` 之类的共享目录中。
