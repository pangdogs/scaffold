...
func (s *_Script) loadSolution() (*dynamic.Solution, error) {
	...
	for _, project := range s.options.Projects {
		if err := solution.Load(project); err != nil {
			return nil, fmt.Errorf("load project %s failed, %s", fmt.Sprintf("%q -> %q", project.ScriptRoot, project.LocalPath), err)
		}
	}
	...
	go func() {
		...
		for {
			...
			case e, ok := <-watcher.Events:
				...
				log.Infof(s.svcCtx, "auto hotfix solution %q detecting %q -> %q changes, preparing to reload in 10s", s.options.PkgRoot, e.Op, e.Name)
